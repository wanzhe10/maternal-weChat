...
function getAreaDataNumberForDetails(provinces,citys,countys,getNumber){
    var tempProvinces = "";
    var tempCitys = "";
    var tempCountys = "";
    if (provinces != null && provinces.length != 0) {
        for (var i = 0; i < iosProvinces.length; i++) {
            if (iosProvinces[i].value == provinces) {
                tempProvinces = iosProvinces[i].id;
                break;
            }            
        }
    }
    if (provinces != null && provinces.length != 0 && citys != null && citys.length != 0) {
        for (var i = 0; i < iosCitys.length; i++) {
            if (iosCitys[i].value == citys && tempProvinces == iosCitys[i].parentId) {
                tempCitys = iosCitys[i].id;
                break;
            } 
        }
    }
    if (provinces != null && provinces.length != 0 && citys != null && citys.length != 0 && countys != null && countys.length != 0) {
        for (var i = 0; i < iosCountys.length; i++) {
            if (iosCountys[i].value == countys && tempCitys == iosCountys[i].parentId) {
                tempCountys = iosCountys[i].id;
                break;
            } 
        }
    }
    getNumber(tempProvinces,tempCitys,tempCountys);
}